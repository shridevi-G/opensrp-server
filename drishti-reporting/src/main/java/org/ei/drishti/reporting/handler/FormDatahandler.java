...
public class FormDatahandler {
	...
	private static Logger logger = LoggerFactory
			.getLogger((String) FormDatahandler.class.toString());
	String regNumber = "";
	String wifeName = "";
	String phoneNumber = "";
	String village = "";
	...
	public void ecRegistration(JSONObject dataObject, String anmPhoneNumber)
		...
		if (!currentMethod.equalsIgnoreCase("none")
				&& !currentMethod.equalsIgnoreCase("none_ps")
				&& !currentMethod.equalsIgnoreCase("none_ss")
				&& !currentMethod.equalsIgnoreCase("dmpa_injectable")
				&& !currentMethod.equalsIgnoreCase("traditional_methods")
				&& !currentMethod.equalsIgnoreCase("centchroman")) {
			ancVisitRepository.reportinsert("", entityId, wifeName, anmid,
					"FP", currentMethod, numberOfCondomsSupplied,
					registrationDate, village, 0, "", "", "");
		}
	...
	public void recordECP(JSONObject dataObject, String anmPhoneNumber)
			throws JSONException {
		String entityId = dataObject.getString("entityId");
		String anmid = dataObject.getString("anmId");
		...
		ancVisitRepository.reportinsert("", entityId, wifeName, anmid, "FP",
				"ecp", numberOfECPsGiven, submissionDate, village, 0, "", "",
				"");
	}

	public void ecEdit(JSONObject dataObject, String anmPhoneNumber)
			throws JSONException {
		String entityId = dataObject.getString("entityId");
		...
		String sid = collect(ancregdetails, on(EcRegDetails.class).id()).get(0)
				.toString();
		String ptphoneNumber = collect(ancregdetails,
				on(EcRegDetails.class).phonenumber()).get(0).toString();
	...
	public void ancEdit(JSONObject dataObject, String anmPhoneNumber)
			throws JSONException {
		String entityId = dataObject.getString("entityId");
		String ecId = "";
		...
		String sid = collect(ancregdetails, on(ANCVisitDue.class).id()).get(0)
				.toString();
		String ptphoneNumber = collect(ancregdetails,
	...
	public void ancRegistration(JSONObject dataObject, String visittype,
		...
		String visittime = collect(visitconf,
				on(VisitConf.class).anc_visit1_from_week()).get(0).toString();
		...
		if (visittype.equalsIgnoreCase("anc_registration")) {
			...
			String ptphoneNumber = collect(ancvisitdetails,
					on(EcRegDetails.class).phonenumber()).get(0).toString();
		

			ancVisitRepository.insert(entityId, ptphoneNumber, anmNumber,
					"anc_visit", visitnumber, edd, wifeName, visitdate, anmid);
			ancVisitRepository.reportinsert("", entityId, wifeName, anmid,
					"anc", "", 0, registrationDate, village, 0, edd, "", "");
		}
		if (visittype.equalsIgnoreCase("anc_registration_oa")) {
	
			smsController.sendSMSEC(phoneNumber, regNumber, wifeName, "ANC");
		
			ancVisitRepository.insert(entityId, phoneNumber, anmNumber,
					"anc_visit", visitnumber, edd, wifeName, visitdate, anmid);
			ancVisitRepository.reportinsert("", entityId, wifeName, anmid,
					"anc", "", 0, registrationDate, village, 0, edd, "", "");
		}
	...
	public void ttData(JSONObject dataObject, String visittype)
			throws JSONException {
		String anmid = dataObject.getString("anmId");
		String entityId = dataObject.getString("entityId");
		String ecId = "";
		...
		ancVisitRepository.reportinsert(entityId, ecId, "", anmid, ttDose, "",
				0, ttDate, village, 0, "", ttInjectionPlace, "");
	}

	public void ancVisit(JSONObject dataObject, String visittype)
			throws JSONException {
		...
		String datetime = collect(ancvisitdetails,
				on(ANCVisitDue.class).lmpdate()).get(0).toString();
		String visit = collect(ancvisitdetails, on(ANCVisitDue.class).visitno())
				.get(0).toString();
		int visitnum = Integer.parseInt(visit);
		switch (visitnum) {
		case 1:
			visittime = collect(visitconf,
					on(VisitConf.class).anc_visit2_from_week()).get(0)
					.toString();
		...
		case 2:
			visittime = collect(visitconf,
					on(VisitConf.class).anc_visit3_from_week()).get(0)
					.toString();
		...
		case 3:
			visittime = collect(visitconf,
					on(VisitConf.class).anc_visit4_from_week()).get(0)
					.toString();
		...
		String sid = collect(ancvisitdetails, on(ANCVisitDue.class).id())
				.get(0).toString();
	...
	public void pncRegistration(JSONObject dataObject, String visittype,
			String anmNumber) throws JSONException {
		String child_dob = "";
		...
		JSONArray fieldJsonArray = dataObject.getJSONObject("formInstance")
				.getJSONObject("form").getJSONArray("fields");
		JSONArray subfieldJsonArray = dataObject.getJSONObject("formInstance")
				.getJSONObject("form").getJSONArray("sub_forms");
	
		JSONArray subfieldJsonArr = subfieldJsonArray.getJSONObject(0)
				.getJSONArray("instances");
	
		logger.info("weight child:  "
				+ subfieldJsonArr.getJSONObject(0).getString("weight"));
		if (!subfieldJsonArr.getJSONObject(0).getString("weight")
				.equalsIgnoreCase("")) {
			String chiweight = subfieldJsonArr.getJSONObject(0).getString(
					"weight");
			child_weight = Integer.parseInt(chiweight);
		}
		String immunizations = subfieldJsonArr.getJSONObject(0).getString(
				"immunizationsGiven");
		...
		for (int i = 0; i < fieldJsonArray.length(); ++i) {
			...
			if (jsonObject.has("name")
				...
				child_dob = jsonObject.has("value")
						&& jsonObject.getString("value") != null ? jsonObject
						.getString("value") : "";
		...
		if (visittype.equalsIgnoreCase("pnc_registration_oa")) {
			logger.info("phonenumber" + phoneNumber + "*** wife name"
					+ wifeName + "***reg Number" + regNumber);
	...
	public void visitpoc(JSONObject dataObject, String visittype)
			throws JSONException {
		String entityidEC = "";
		String motherName = "";
		...
		for (int i = 0; i < fieldsJsonArray.length(); i++) {
			...
			if ((jsonObject.has("name"))
				...
				if (isCon.equalsIgnoreCase("yes")) {
					...
					String sid = collect(subcenterForANM,
							on(ANMVillages.class).subcenter()).get(0)
							.toString();
					logger.info("subcenter from db:" + sid);
					Integer id = Integer.parseInt(sid);
					List phcdetails = anmService.getPHCDetails(id);
					String phcname = collect(phcdetails,
							on(HealthCenter.class).parent_hospital()).get(0)
							.toString();
					logger.info("phc name from healthcenters" + phcname);
					String hospitaltype = collect(phcdetails,
							on(HealthCenter.class).hospital_type()).get(0)
							.toString();
					...
					if (hospitaltype.equalsIgnoreCase("Subcenter")) {
						if (visittype.equalsIgnoreCase("child_illness")) {
							ancVisitRepository.pocinsert(visittype,
									visitentityid, entityidEC, anmid, phcname,
									date, motherName);
						}
	...
	public void child_illness(JSONObject dataObject, String visittype)
			throws JSONException {
		String entityId = dataObject.getString("entityId");
		String anmid = dataObject.getString("anmId");
		String childDisease = "";
		...
		if (childDisease.equalsIgnoreCase("diarrhea")
				|| childDisease.equalsIgnoreCase("diarrhea_dehydration")) {
			ancVisitRepository.reportinsert("", entityId, motherName, anmid,
					"child_illness", childDisease, 0, childDiseasereportDate,
					village, 0, "", "", "");
		}
	}

	public void childRegistration(JSONObject dataObject, String visittype,
			String anmNumber) throws JSONException {
		...
		if (visittype.equalsIgnoreCase("child_registration_oa")) {
			...
			ancVisitRepository.insert(entityId, phoneNumber, anmNumber,
					"child_Immunization", visitnumber, dateOfBirth, motherName,
					edd, anmid);

			ancVisitRepository.reportinsert("", entityId, motherName, anmid,
					"child_oa", immunizationsGiven, 0, registrationDate,
					village, birthWeight, "", "", dateOfBirth);
		}
		if (visittype.equalsIgnoreCase("child_registration_ec")) {
			List ancvisitdetails = anmService.getPhoneNumber(entityId);

			String ptphoneNumber = collect(ancvisitdetails,
					on(EcRegDetails.class).phonenumber()).get(0);
			logger.info("phonenumber: " + ptphoneNumber);
			smsController.sendSMSChild(ptphoneNumber, wifeName);
			ancVisitRepository.insert(childId, ptphoneNumber, anmNumber,
					"child_Immunization", visitnumber, dateOfBirth, wifeName,
					edd, anmid);
			ancVisitRepository.reportinsert("", entityId, wifeName, anmid,
					"child", immunizationsGiven, 0, registrationDate, village,
					birthWeight, "", "", dateOfBirth);
		}

	}

	public void childImmunization(JSONObject dataObject, String visittype)
			throws JSONException {
		String motherName = "";
		...
		ancVisitRepository.reportinsert("", entityId, motherName, anmid,
				"child_immunization", immunizationsReceived, 0,
				registrationDate, village, 0, immunizationDate,
				immunizationPlace, "");
	}
}