	...
	public void formData(@RequestBody List<FormSubmissionDTO> formSubmissionsDTO)
		...
		while (itr.hasNext()) {
			...
			case "anc_registration":
				formDataHandler.ancRegistration(dataObject, visittype,
						anmphoneNumber);
				break;
			case "anc_close":
			...
			case "delivery_outcome":
				String phoneNumber = anmService.getanmPhoneNumber(user_id)
						.toString();
				formDataHandler.pncRegistration(dataObject, visittype,
						phoneNumber);
				break;

			case "pnc_registration_oa":
				String phoneNumber1 = anmService.getanmPhoneNumber(user_id)
						.toString();
				formDataHandler.pncRegistration(dataObject, visittype,
						phoneNumber1);
				break;
