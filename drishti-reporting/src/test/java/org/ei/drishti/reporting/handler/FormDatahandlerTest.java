...
//		JSONObject sub_forms = new JSONObject();
//		sub_forms.put("fields", list);
//		formInstance.put("sub_forms", form);
	...
	public void pncRegistrationtest() throws Exception {
		...
		ANCVisitDue ancvisitDue = new ANCVisitDue(2311,"entityid", "patientnum",
				"anmnum", "visittype", 12312, "lmpdate", "womenname",
				"visitdate", "anmid");
	
		when(visitService.getVisitDue("342")).thenReturn(asList(ancvisitDue));
	...
	public void visitpoctest() throws Exception {
		...
		ANMVillages anmvillages = new ANMVillages(324, "villages", "user_role",
		...
		HealthCenter healthCenter =new HealthCenter(234234,"hospital_name", "hospital_type", "hospital_address","parent_hospital","villages",
	    		324,324,324,23234);
		when(anmService.getPHCDetails(123)).thenReturn(
				asList(healthCenter));

		formDatahandler.visitpoc(dataobject, "Subcenter", "12645");
		formDatahandler.visitpoc(dataobject, "anc_visit", "12645");
